...
exports.register = async (req, res) => {
    try {
        const {username, password, phoneNumber, firstName, lastName,  email, role, gender} = req.body;
        if (!username || !password || !phoneNumber || !firstName || !lastName || !email || !gender)
            return res.status(400).json({message: 'Missing required fields'});
        const existingUser = await User.findOne({$or: [{username}, {phoneNumber}, {email}]});
        if (existingUser)
            return res.status(409).json({message: 'Username or phoneNumber or email already taken'});
...
exports.updateProfile = async (req, res) => {
    try {
        ...
        const allowedUpdates = ['firstName', 'lastName', 'username', 'phoneNumber', 'address'];
        ...
        for (let key of updates) {
            if (key === 'username') {
                ...
                const existingUser = await User.findOne({phoneNumber: key});
                if (existingUser)
